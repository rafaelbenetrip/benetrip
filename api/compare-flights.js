...
// Atualizado com suporte a crianças, bebês e correção de renderização
...
export default async function handler(req, res) {
    ...
    try {
        const { origem, destino, datasIda, datasVolta, moeda, passageiros } = req.body;

        if (!origem || !/^[A-Z]{3}$/i.test(origem)) return res.status(400).json({ error: 'Origem inválida (IATA 3 letras)' });
        if (!destino || !/^[A-Z]{3}$/i.test(destino)) return res.status(400).json({ error: 'Destino inválido (IATA 3 letras)' });
        if (!Array.isArray(datasIda) || datasIda.length === 0 || datasIda.length > MAX_IDAS) return res.status(400).json({ error: `Informe 1 a ${MAX_IDAS} datas de ida` });
        if (!Array.isArray(datasVolta) || datasVolta.length === 0 || datasVolta.length > MAX_VOLTAS) return res.status(400).json({ error: `Informe 1 a ${MAX_VOLTAS} datas de volta` });

        const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
        for (const d of [...datasIda, ...datasVolta]) {
            if (!dateRegex.test(d)) return res.status(400).json({ error: `Data inválida: ${d} (use YYYY-MM-DD)` });
        }
        ...
        // Tratamento seguro dos passageiros
        const numAdultos = Math.min(Math.max(parseInt(passageiros?.adultos) || 1, 1), MAX_PASSENGERS);
        const numCriancas = Math.min(Math.max(parseInt(passageiros?.criancas) || 0, 0), MAX_PASSENGERS);
        const numBebes = Math.min(Math.max(parseInt(passageiros?.bebes) || 0, 0), numAdultos); // Bebês <= Adultos
        ...
        if (combinacoes.length === 0) {
            return res.status(400).json({ error: 'Nenhuma combinação válida', message: 'As datas de volta devem ser posteriores às de ida' });
        }
        ...
        for (let i = 0; i < combinacoes.length; i += BATCH_SIZE) {
            ...
            const promises = batch.map((combo, idx) => {
                ...
                const params = {
                    ...
                    children: String(numCriancas),
                    infants_on_lap: String(numBebes)
                };
        ...
        return res.status(200).json({
            ...
            destino: destinoCode,
            moeda: currencyCode,
            passageiros: { adultos: numAdultos, criancas: numCriancas, bebes: numBebes },
            datasIda,
            datasVolta,
            stats,
            matrizPrecos,
