...
| `api/_lib/geo-routes.js` | coordenadas e deslocamento aeroporto→destino | Google Geocoding; Google Directions | `GOOGLE_PLACES_API_KEY` ou `GOOGLE_API_KEY` |
...
| Escapadas / Destinos Baratos / Todos | Geocoding/Directions indisponível | o card diz que existe deslocamento terrestre a partir do aeroporto, sem número de km nem tempo |
...
| geocodificação | 7 dias | coordenadas quase não mudam |
| rota terrestre | 24 h | trânsito muda, a rota não |
