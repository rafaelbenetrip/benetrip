/**
 * Benetrip - Sistema de Roteiro Contínuo Otimizado (VERSÃO 8.0)
 * Características: Roteiro contínuo sem divisões, 100% mobile, imagens responsivas
 * Data: 2025
 */
...
const BENETRIP_ROTEIRO = {
   ...
   */
  init() {
    console.log('🚀 Benetrip Roteiro v8.0 - Roteiro Contínuo Inicializando...');
  ...
  /**
   * ✅ NOVO: Configura lazy loading para imagens
   */
  ...
  /**
   * ✅ PRINCIPAL: Gera roteiro contínuo sem divisões
   */
  async gerarRoteiroContinuo() {
    try {
      ...
      this.roteiroPronto = await this.gerarRoteiroContinguoDummy(dataIda, dataVolta, diasViagem);
  ...
  /**
   * ✅ NOVO: Gera roteiro contínuo sem divisões de horário
   */
  async gerarRoteiroContinguoDummy(dataIda, dataVolta, diasViagem) {
    console.log('🏗️ Gerando roteiro contínuo...');
    ...
    for (let i = 0; i < diasViagem; i++) {
      ...
      const dia = {
        ...
        atividades: this.gerarAtividadesDoDia(i, destino, atividadesBase, diasViagem)
      };
  ...
  obterAtividadesContinuas(destino) {
    ...
    // Atividades específicas por destino
    const especificos = {
      'Lisboa': [
        ...
        { local: "Almoço no Time Out Market", dica: "Variedade incrível de sabores!", horario: "13:30" },
  ...
  /**
   * ✅ NOVO: Gera atividades específicas para cada dia
   */
  gerarAtividadesDoDia(diaIndex, destino, atividadesBase, totalDias) {
    const atividades = [];
    
    // Número de atividades por dia (varia entre 4-7)
    const numAtividades = 4 + (diaIndex % 4);
    
    // Calcular índice inicial baseado no dia
    ...
    for (let i = 0; i < numAtividades && inicioIndex + i < atividadesBase.length; i++) {
      ...
      atividadeBase.duracao = this.estimarDuracao(atividadeBase.local);
      
      // Adicionar variação no horário para não ficar repetitivo
      if (atividadeBase.horario) {
        atividadeBase.horario = this.ajustarHorario(atividadeBase.horario, i);
      }
  ...
  /**
   * ✅ NOVO: Ajusta atividades do primeiro e último dia (versão contínua)
   */
  ...
  /**
   * Busca previsão do tempo (limitada a 3 dias)
   */
  ...
  /**
   * ✅ ATUALIZADO: Atualiza UI com roteiro contínuo
   */
  ...
  /**
   * ✅ NOVO: Cria elemento de dia com atividades contínuas
   */
  ...
  // Resto dos métodos helper (sem mudanças significativas)
  ...
  // [Incluir todos os outros métodos auxiliares do código original...]
  // Por brevidade, vou incluir apenas os essenciais aqui
  ...
  // Métodos de dados, utilidades e ações mantidos
  extrairDataIda() { /* [código original] */ },
  extrairDataVolta() { /* [código original] */ },
  garantirFormatoISO(dataInput) { /* [código original] */ },
  formatarDataISO(data) { /* [código original] */ },
  calcularDiasViagem(dataIda, dataVolta) { /* [código original] */ },
  getDataIda() { return this.dadosVoo?.infoIda?.dataPartida; },
  getDataVolta() { return this.dadosVoo?.infoVolta?.dataPartida; },

  obterPreferencias() { /* [código original] */ },
  obterNomeDestinoPorCodigo(codigo) { /* [código original] */ },
  obterPaisPorCodigo(codigo) { /* [código original] */ },
  formatarData(dataString) { /* [código original] */ },
  formatarDataCompleta(dataString) { /* [código original] */ },
  ...
  // Métodos de carregamento e progresso
