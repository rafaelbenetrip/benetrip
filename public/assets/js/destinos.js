...
mostrarDestinoSurpresa() {
  ...
  modalContainer.innerHTML = `
    <div class="bg-white rounded-lg w-full max-w-md relative max-h-[90vh] overflow-hidden transform transition-transform duration-500 modal-surpresa-content">
      <!-- Imagem com banner e botão de fechar (altura reduzida) -->
      <div class="relative">
      ...
      <!-- Título do destino (fontes reduzidas) -->
      ...
      <!-- Conteúdo da aba Informações (fontes e espaçamentos reduzidos) -->
      <div id="conteudo-surpresa-info" class="conteudo-aba-surpresa p-3 overflow-y-auto" style="max-height: calc(90vh - 240px);">
        <div class="bg-gray-50 p-2 rounded-lg">
          <div class="flex items-center mb-1">
            <span class="text-sm mr-2">✈️</span>
            <span class="font-medium text-sm">Aeroporto</span>
          </div>
          <p class="font-medium text-sm">${destino.aeroporto?.codigo || 'N/A'} - ${destino.aeroporto?.nome || `Aeroporto de ${destino.destino}`}</p>
        </div>
        ...
        <div class="mt-3 bg-gray-50 p-2 rounded-lg">
          ...
          <p class="font-medium text-sm">${this.obterDatasViagem()}</p>
        </div>
        
        ${destino.porque ? `
          <div class="mt-3 bg-yellow-50 p-2 rounded-lg border border-yellow-100">
            <div class="flex items-start">
              <span class="text-sm mr-2">🎁</span>
              <div>
                <h4 class="font-medium mb-1 text-sm">Por que visitar:</h4>
        ...
        ${destino.destaque ? `
          <div class="mt-3">
            <h4 class="font-medium mb-1 text-sm">Destaque da experiência:</h4>
            <p class="text-gray-800 text-sm">${destino.destaque}</p>
          </div>
        ` : ''}
      </div>
      
      <!-- Conteúdo da aba Pontos Turísticos (fontes e espaçamentos reduzidos) -->
      <div id="conteudo-surpresa-pontos" class="conteudo-aba-surpresa p-3 overflow-y-auto hidden" style="max-height: calc(90vh - 240px);">
        ${destino.pontosTuristicos && destino.pontosTuristicos.length > 0 ? 
          destino.pontosTuristicos.map((ponto, idx) => {
            const imagem = this.encontrarImagemParaPontoTuristico(destino.imagens, ponto, idx);
            return `
              <div class="bg-white border border-gray-200 rounded-lg p-2 mb-2 shadow-sm">
                <div class="flex items-center">
                  <span class="flex items-center justify-center w-6 h-6 rounded-full mr-2 text-white font-bold text-xs" style="background-color: #00A3E0;">${idx + 1}</span>
                  <h5 class="font-medium text-sm">${ponto}</h5>
                </div>
                
                ${imagem ? `
                  <div class="mt-2 ml-8 rounded-lg overflow-hidden h-20">
                    ${this.renderizarImagemComCreditos(imagem, ponto, 'h-full w-full', { showPontoTuristico: false })}
                  </div>
                ` : ''}
              </div>
          ...
          '<p class="text-center text-gray-500 my-4 text-sm">Informações sobre pontos turísticos não disponíveis</p>'
        }
      </div>
      
      ${destino.clima && destino.clima.temperatura ? `
        <!-- Conteúdo da aba Clima - APENAS DADOS DA LLM (fontes e espaçamentos reduzidos) -->
        <div id="conteudo-surpresa-clima" class="conteudo-aba-surpresa p-3 overflow-y-auto hidden" style="max-height: calc(90vh - 240px);">
          <div class="text-center bg-blue-50 p-3 rounded-lg">
            <h4 class="font-medium text-base mb-1">Clima durante sua viagem</h4>
            <div class="text-2xl mb-1">🌤️</div>
            
            ${destino.clima.estacao ? `
              <p class="text-base font-bold">${destino.clima.estacao}</p>
            ` : ''}
            
            ${destino.clima.temperatura ? `
              <p class="text-xs text-gray-600 mt-1">Temperatura: ${destino.clima.temperatura}</p>
            ` : ''}
            
            ${destino.clima.condicoes ? `
              <p class="text-xs text-gray-600 mt-1">${destino.clima.condicoes}</p>
            ` : ''}
          </div>
          
          ${destino.clima.recomendacoes ? `
            <div class="mt-3 bg-white border border-gray-200 rounded-lg p-2">
              <h5 class="font-medium mb-1 text-sm">Recomendações:</h5>
              ${Array.isArray(destino.clima.recomendacoes) ? 
                `<ul class="list-disc pl-4 text-xs text-gray-700 space-y-0.5">
                  ${destino.clima.recomendacoes.map(rec => `<li>${rec}</li>`).join('')}
                </ul>` :
                `<p class="text-xs text-gray-700">${destino.clima.recomendacoes}</p>`
              }
            </div>
          ` : ''}
          
          <div class="mt-3 bg-green-50 p-2 rounded-lg border border-green-200">
            <div class="flex items-center">
              <span class="text-sm mr-1">🤖</span>
              <span class="text-xs font-medium text-green-800">Informações climáticas fornecidas pela IA especializada</span>
            </div>
          </div>
        </div>
      ` : ''}
...
<div id="conteudo-surpresa-comentarios" class="conteudo-aba-surpresa p-3 overflow-y-auto hidden" style="max-height: calc(90vh - 240px);">
  ...
  ` : ''}
      ...
      <div class="p-3 border-t border-gray-200">
        ...
        <button class="w-full font-medium py-2 px-4 rounded-lg border border-gray-300 transition-colors duration-200 hover:bg-gray-100" 
          onclick="document.getElementById('modal-surpresa').remove()">
          Voltar às Sugestões
        </button>
      </div>
